...
Future<void> scheduleSleepNotifications() async {
  ...
  bool startedDailyCheckIns = diaryReminderTimes != null;

  if (!startedDailyCheckIns) {
    if (s == "IntroductionWelcome" ||
      ...
      int sleepEfficiencyPercent =
          ((sleepDuration / bedDuration) * 100).round();
      await serviceLocator<NotificationService>().showContentReminder(
          baseId + 3 * 10 + 1,
          "sleep_content",
      ...
      int sleepMedication =
          int.tryParse(kvReadStringList("sleep", "how-sleep-medication")[0]) ??
              0;
...
// Set #16
  ...
  if (!s.startsWith("Done") && startedDailyCheckIns && !progress.todayDone) {
    // Set #17
  ...
  if (s != "DoneEnd" && startedDailyCheckIns && progress.allDone) {
    // Set #18
    await serviceLocator<NotificationService>().showContentReminder(
        baseId + 18 * 10 + 1,
        "sleep_content",
